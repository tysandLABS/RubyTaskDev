pipeline {
  ...
  environment{
   ...
   stages {
    ...
    stage ('Build') {
      steps {
          sh 'sudo cp /home/ubuntu/Dockerfile . && docker build -t tsanderson77/tasks_app:v1 . && docker scout quickview > dockerscout.txt && docker scout recommendations local://tsanderson77/tasks_app:v1 > dockerscout2.txt'
      }
}
     stage ('Login') {
        steps {
